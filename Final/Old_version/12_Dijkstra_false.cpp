...
struct _Set** run(_Vehicle* start);       //run from the start;
...
int main(){
    ...
    make_connect_loop();
    _Set** S_set = run(start);
    //output_file( S_set);
    //printf("\n~~");
    while( S_run( S_set ) )     system("Pause");
    return 0;
}

//~ ~ ~ ~ ~ ~ ~ ~ ~ ~functions~ ~ ~ ~ ~ ~ ~ ~ ~ ~//
//run
struct _Set** run(_Vehicle* start){
...
//OUTPUT file
// void output_file(_Set** S_set){
//     FILE* ouf = fopen("out12.txt","w");
//     if(!ouf){   printf("\nError!\n");   exit(1);}
//     for(int i = 0; i < Y_width; ++i){
//         for(int j = 0; j < X_width; ++j){
//             for(int face = 0; face < 4; ++face){
//                 if(!(*(S_set + i*X_width*4 + j*4 + face ))){
//                     fprintf(ouf,"-1\n");
//                     continue;
//                 }
//                 fprintf(ouf,"%d ",(*(S_set + i*X_width*4 + j*4 + face ))->fuel_consumption);
//                 _StepLog* tmp = (*(S_set + i*X_width*4 + j*4 + face ))->step_log;
//                 while(tmp){
//                     fprintf(ouf,"%d ",tmp->car->driver->y);
//                     fprintf(ouf,"%d ",tmp->car->driver->x);
//                     fprintf(ouf,"%d ",tmp->car->face);
//                     tmp = tmp->last;
//                 }
//                 fprintf(ouf,"\n");
//             }
