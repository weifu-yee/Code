...
void throw_S(_Set** Q_set, _Set** S_set, int _i, int _j);
void catch_Q( _Set** Q_set, int* _i, int* _j);
...
bool S_run(_Set** S_set){
    ...
    for(int i = 0; i < Y_width; ++i){
        for(int j = 0; j < X_width; ++j){
            if( !(*(S_set + X_width*i + j )))       continue;
            if( (*(S_set + X_width*i + j ))->visited )       continue;
            if(!c && o >= (*(S_set + X_width*i + j ))->fuel_consumption)        continue;
            if(c)   c = false;
            tmp = (*(S_set + X_width*i + j ));
...
void recursionUpdate_S(_Set** S_set, _StepLog* curr){
    ...
    for( int l = 0; l < 6; l ++){   //see every vertex's fuel;
        ...
        if(!(*(S_set + X_width*i_prime + j_prime )))        continue;
