...
int* RNG(int* arr){     //to generate random number
    ...
    while(cnt < 4){
        do{
            num = rand() % 10;
        }while( cnt == 0 && num == 0 || num == arr[0] || num == arr[1] || num == arr[2] || num == arr[3]);
        arr[cnt++] = num;
    }
    int* arrptr = arr;
    return arrptr;
...
void run( int* arr){
    ...
    printf("Please input 4 numbers to start the game!\n");
    do{
        f = true;
        A = 0, B = 0;       cnt ++;
        printf("    Guess(%d):\t",cnt);
        fflush(stdin);
        if( scanf("%1d%1d%1d%1d",&num[0],&num[1],&num[2],&num[3]) != 4){
            fflush(stdin);
        ...
        printf("You're input is---------->'%d%d%d%d' ~ ~ '%dA %dB'\n",num[0],num[1],num[2],num[3],A,B);
    ...
    for(int k = 0; k < 30; k ++){    printf("-");   Sleep(30);}
    puts("");
    printf("\n>> The answer is '%d%d%d%d'!",arr[0],arr[1],arr[2],arr[3]);
    Sleep(1000);
